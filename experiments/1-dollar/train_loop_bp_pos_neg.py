...
from data_preprocessing import prepare_dataloaders_per_classes
...
def main():
    human_dir = '../../datasets/csv-1dollar-human'
    synth_dir = '../../datasets/csv-1dollar-synth-best'

    classes = ['arrow', 'caret', 'check', 'circle', 'delete_mark', 'left_curly_brace',
    ...
    try:
        train_loader, val_loader, test_loader = prepare_dataloaders_per_classes(
            human_dir, synth_dir, classes, batch_size=600, max_length=sequence_length, test_size=0.2, val_size=0.1)
    ...
    model.load_state_dict(torch.load('models/BP_best_model.pth', weights_only=True))
