...
class MobileTouchDataset(Dataset):
    def __init__(self, human_dir, synth_dir, transform=None, max_length=70):
        self.human_files = self._get_files(human_dir)
        self.synth_files = self._get_files(synth_dir)
    ...
    def _get_files(self, directory):
        pattern = os.path.join(directory, f"*.csv")
    ...
    def _load_sequence(self, file_path):
        try:
            ...
            sequence = df[['stroke_id', 'x', 'y', 'time']].values
            ...
            # print('preprocessed sequence', sequence)
...
def prepare_dataloaders(human_dir, synth_dir, batch_size=256, max_length=70, test_size=0.2, val_size=0.1):
    full_dataset = MobileTouchDataset(
        human_dir=human_dir,
        synth_dir=synth_dir,
